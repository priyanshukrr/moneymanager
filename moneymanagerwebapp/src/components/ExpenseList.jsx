import { Download, LoaderCircle, Mail, Receipt } from "lucide-react";
    ...
    return (
        <div className="card bg-white p-6 rounded-2xl border border-slate-100/80 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                    <h5 className="text-base font-bold text-slate-900">All Expenses</h5>
                    <p className="text-xs text-slate-500 font-medium mt-0.5">Manage and export your expense records</p>
                </div>
            ...
            {(!transactions || transactions.length === 0) ? (
                <div className="py-12 text-center text-slate-400 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
                    ...
                    <p className="text-sm font-semibold text-slate-600">No expense transactions recorded yet</p>
                    <p className="text-xs text-slate-400 mt-1">Click "Add Expense" above to log your first payment.</p>
                </div>
            ) : (
